...
class RiskManager:
    ...
    def evaluate(
        ...
        if ask:
            ...
            max_contracts = max(0, min(trade_cap_contracts, daily_cap_contracts, exposure_cap_contracts, event_cap_contracts))
            ...
            if event_cap_contracts <= 0 and "event_date_exposure_limit" not in reasons:
                reasons.append("event_date_exposure_limit")
        return RiskDecision(allowed=not reasons and max_contracts > 0, reasons=reasons, max_contracts=max_contracts)
